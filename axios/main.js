...
function getTodos() {
  ...
  axios
    .get("http://jsonplaceholder.typicode.com/todos?_limit=5", {    //?_limit=5 获取五条数据   
      timeout: 5000
    })
    .then(res => showOutput(res))
    .catch(err => console.error(err));
...
function getData() {
  axios
    ...
    // .then(res => showOutput(res[1]))    res[0]为0时请求接口todos,为1时请求接口posts
    .then(axios.spread((todos, posts) => showOutput(posts)))    //请求分发
    .catch(err => console.error(err));
}
  ...
  axios
    ...
    .catch(err => {
      if (err.response) {
        ...
        console.error(err.request); //      仅限于请求已经发起但没有响应才会执行这部分内容
      } else {
        console.error(err.message);//       请求也没法起响应也没有，与后端相关
      }
    });
}
...
function cancelToken() {
  const source = axios.CancelToken.source();//      根据某种条件触发取消当前请求
